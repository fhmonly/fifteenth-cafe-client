            ...
            <div class="flex-1 ml-4">
                ...
                <div class="flex items-center justify-start mt-2">
                    <button id="decrement" class="bg-[#A91D3A] text-white px-2 rounded aspect-square h-[29px]"
                        @click="decrement()" @focusout="updateCartQty()">
                        -
                    </button>
                    <input id="quantity" type="number"
                        class="w-10 text-center bg-transparent border rounded text-md no-spin-btn" :value="qty"
                        min="1" />
                    <button id="increment" class="bg-[#A91D3A] text-white px-2 rounded aspect-square h-[29px]"
                        @click="increment()" @focusout="updateCartQty()">
                        +
                    </button>
                </div>
            </div>
...
const { showModalCheckout, removeItemFromCart, cartItems, fetchCartDetails } = useCart()
